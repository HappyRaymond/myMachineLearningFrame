...
def train_test_split():
    ...
    for i in range(len(filePathList)):
        ...
        if (filePathList[i].find("Economise") != -1  and filePathList[i].find("Hong Kong Island") != -1):
            Economise_HK_Data = pd.read_csv(filePathList[i])
        elif (filePathList[i].find("Economise") != -1  and filePathList[i].find("Kowloon") != -1):
            Economise_KL_Data = pd.read_csv(filePathList[i])
        elif (filePathList[i].find("Economise") != -1  and filePathList[i].find("New Territories") != -1):
...
"""
 ...
 CH_I KS PA PKS DP AF LR1 LR2 LR3 EP BS
 
 
"""

channel_num_list = [ [4,8,16],[8,16,32],[8,32,32], [8,16], [16,32], [8,16,16,32], [16,16,32,32]]
...
class MyProblem(ea.Problem):  # 继承Problem父类
    def __init__(self, PoolType,save_path, X_train, y_train, X_test, y_test):  # PoolType是取值为'Process'或'Thread'的字符串
        name = 'CNN_1d'  # 初始化name（函数名称，可以随意设置）
        M = 1  # 初始化M（目标维数）
        maxormins = [-1]  # 初始化maxormins（目标最小最大化标记列表，1：最小化该目标；-1：最大化该目标）
        Dim = 11  # 初始化Dim（决策变量维数）
        #          HL, LR1, LR2, LR3, EP,BS, DP, WD, AF
        varTypes = [1, 1, 1, 1, 0, 1, 0, 0, 0, 1, 1]  # 初始化varTypes（决策变量的类型，元素为0表示对应的变量是连续的；1表示是离散的）
        lb = [0, 3, 0, 1, 0, 0, 1e-4, 1e-5, 1e-6, 2000, 128]  # 决策变量下界
        ub = [len(channel_num_list) - 1, 5, 3, 2, 0.8, len(activate_function_list) - 1, 5e-2, 5e-2, 5e-2, 20000,512]  # 决策变量上界
        ...
        if self.PoolType == 'Thread':
            ...
            self.num_cores = int(mp.cpu_count()/3)  # 获得计算机的核心数
            self.pool = ProcessPool(self.num_cores)  # 设置池的大小
...
def subAimFunc(args):
    ...
    CH_I,KS,PA,PKS,DP,AF,LR1,LR2,LR3,EP,BS= Vars[i, 0],Vars[i, 1],Vars[i, 2],Vars[i, 3],Vars[i, 4],Vars[i, 5],Vars[i, 6],Vars[i, 7],Vars[i, 8],Vars[i, 9],Vars[i, 10]
    CH_I = int(CH_I)
    KS = int(KS)
    PA = int(PA)
    PKS = int(PKS)
    ...
    cnn1d = DeepLearningModel.CNN1D(channel_num_list=channel_num_list[CH_I], kernel_size=KS, stride=1, padding=PA, \
                                    Pool_kernel_size=PKS,   \
                                    dropout=DP, activate_function=activate_function_list[AF], featureNum= np.array(X_train).shape[-1])

    GRT = DeepLearningRegression.General_Regression_Training(cnn1d, learning_rate=[LR1, LR2, LR3], epoch=EP,
                                                             use_more_gpu=True, batch_size=BS,
                                                             save_path=save_path)
...
def mainFun( X_train, y_train, X_test, y_test,save_path,PoolType = 'Process'):
    """===============================实例化问题对象==========================="""
...
if __name__ == '__main__':
    ...
    for k, v in dataDict.items():
        print(k)
        X_train, X_test, y_train, y_test = v["X_train"].values, \
         v["X_test"].values, v["y_train"].values, \
         v["y_test"].values
        print(len(X_train), len(X_test), len(y_train), len(y_test),
              os.path.join("./regression/", k, "CNN_1D"))
               
        mainFun(X_train, X_test, y_train, y_test, os.path.join("./regression/", k, "CNN_1D"))
